...
/**
 * Trait ConsoleTools
 * @package Rinvex\Attributes\Traits
 */
trait ConsoleTools
{
    /**
    ...
    protected function publishesMigrations(string $package, bool $isModule = false): void
    {
        ...
        if (file_exists($path = $basePath.'/database/migrations')) {
            ...
            $migrations = collect($stubs)->flatMap(function ($migration) use ($existing, $package) {
                ...
                return [
                    $migration => $this->app->databasePath('migrations/'.$package.'/'.($match ? basename($match) : date('Y_m_d_His',
                                time() + mb_substr($sequence, -6)).str_replace($sequence, '', basename($migration)))),
                ];
            })->toArray();
    ...
    protected function publishesConfig(string $package, bool $isModule = false): void
    {
        ...
        if (file_exists($path = $basePath.'/config/config.php')) {
            $this->publishes([$path => $this->app->configPath(str_replace('/', '.', $namespace).'.php')],
                $namespace.'::config');
        }
    }
    ...
    {
        return $this->publishesResources() && $this->app['config'][str_replace(['laravel-', '/'], ['', '.'],
                $config).'.autoload_migrations'];
    }
}